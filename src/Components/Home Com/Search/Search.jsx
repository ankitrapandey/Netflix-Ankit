...
// // export default Search;
 ...
 {/* <div className='relative'>
                      {visibility && (
                        <div className={`transition-all duration-500 ${visibility ? 'block opacity-100' : 'hidden opacity-0'}`} style={{ position: 'absolute', bottom: '100%', right: '0', zIndex: '999', width: '100%' }}>
                          <div>
                            <input
                              value={searchQuery}
                              onChange={(e) => setSearchQuery(e.target.value)}
                              className='h-[34px] w-[90%] ml-4 mt-1 border border-[#00acd6] text-[14px]'
                              type="text"
                            />
                            <p className='h-[34px] lg:w-[90%] hover:bg-[#3c8dbc] cursor-pointer ml-3 bg-[rgba(214,216,218)] pl-2 pt-2 mt-1 text-[13px] text-[#444444]'> Please select</p>
                            <p></p>
                            {data.map((item, index) => (
                              <p
                                key={index}
                                className='text-[13px] text-[#444444] hover:text-white hover:bg-[#3c8dbc] cursor-pointer pl-3 pt-3'
                      ...
                      <div className='flex items-center h-[34px] lg:w-[13vw] w-full text-[13px] text-[#444444] cursor-pointer border border-[#d2d6de] ml-3 mt-2'>
                        <span className="pl-3 cursor-pointer ">{selectedOption}</span>
                        <i
                          onClick={() => plusVisibility(0)}
                          className={`fa-solid fa-caret-down text-[13px] text-[#444444] cursor-pointer transition-transform ${visibility ? 'rotate-180' : ''}`}
                          style={{ transformOrigin: '50% 50%', position: 'absolute', right: '10px', top: '', transform: visibility ? 'rotate(180deg)' : 'none' }}
                        ></i>
                      </div>
